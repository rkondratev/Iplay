...
# ✅ CORS настройки — разрешаем все origins
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_credentials=False,  # ← False, не True!
    allow_methods=["*"],
    allow_headers=["*"],
)
...
def video_info(video_id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        # ✅ Возвращаем как dict, не как модель (избегаем ошибок Pydantic)
        return {
            "id": video.id,
...
def format_date(dt) -> str:
    ...
    try:
        ...
        # Если dt с timezone — убираем его для сравнения
        if hasattr(dt, 'tzinfo') and dt.tzinfo is not None:
            dt = dt.replace(tzinfo=None)
        
        # Если now с timezone — убираем его
